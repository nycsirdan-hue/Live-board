...
function EntryLine({
  ...
  const itemText = joinItems(items, itemLimit);
...
function DisplayRotationOverlay({ eventDisplay }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-6 left-6 rounded-2xl bg-black/70 px-5 py-3 text-white backdrop-blur">
        <div className="text-lg font-semibold">
          {eventDisplay.eventName}
        </div>

        {eventDisplay.eventDescription ? (
          <div className="mt-1 max-w-3xl text-sm text-slate-200">
            {eventDisplay.eventDescription}
          </div>
        ) : null}
      </div>
...
export default function App() {
  ...
  const [savedEventDisplays, setSavedEventDisplays] = useState(() => {
    try {
      return JSON.parse(window.localStorage.getItem("savedEventDisplays") || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    window.localStorage.setItem("savedEventDisplays", JSON.stringify(savedEventDisplays));
  }, [savedEventDisplays]);
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event) => {
      if (event.key === "activeEventDisplayId" || event.key === "savedEventDisplays") {
  ...
  useEffect(() => {
    if (!isEntryMode || !entrySuccess) return;
  ...
  const saveEventDisplayPreset = async () => {
    ...
    setMessage("Preparing image(s) for saving...");
    const compressedImages = await compressDisplayImages(displayImages);

    const estimatedSize = JSON.stringify(compressedImages).length;
    const maxSafeSize = 3500000;

    if (estimatedSize > maxSafeSize) {
      setMessage(
        "This preset is still too large for browser storage. Try fewer images or smaller image files."
      );
    ...
    if (editingEventDisplayId) {
      ...
      const updatedPreset = {
        ...existingPreset,
        eventName: name,
        eventDescription: displayEventDescription.trim(),
        liveboardDurationSeconds: liveboardDuration,
        transitionSeconds: 0.5,
        images: compressedImages.map((image, index) => ({
          ...image,
      ...
      setSavedEventDisplays((current) =>
        current.map((eventDisplay) =>
          eventDisplay.id === editingEventDisplayId ? updatedPreset : eventDisplay
        )
      );

      setActiveEventDisplayId(updatedPreset.id);
    ...
    setSavedEventDisplays((current) => [...current, newPreset]);
    setActiveEventDisplayId(newPreset.id);
    ...
    setMessage("Event display preset saved.");
    setTimeout(() => setMessage(""), 2000);
  };
  ...
  const deleteActiveEventDisplayPreset = () => {
    ...
    const confirmMessage = isBuiltinPreset
      ...
      : `Delete event display preset "${activePreset.eventName}"?`;
    ...
    if (isBuiltinPreset) {
      ...
      setActiveEventDisplayId(nextOptions[0]?.id || "");
    ...
    const visibleBuiltins = eventDisplayPresets.filter(
      (eventDisplay) => !hiddenBuiltinEventDisplayIds.includes(eventDisplay.id)
    );

    setSavedEventDisplays(remaining);
    setActiveEventDisplayId(visibleBuiltins[0]?.id || remaining[0]?.id || "");
    ...
    setMessage("Event display preset deleted.");
    setTimeout(() => setMessage(""), 2000);
  };
  ...
  const createEntry = async () => {
    ...
    const finalWhoAmI = whoAmIInput.trim();
    const finalSeeking = seekingInput.trim();

    if (!finalWhoAmI) {
      setMessage("Please enter who you are.");
      return;
    }

    if (!finalSeeking) {
      setMessage("Please enter who you are seeking.");
      return;
    }

    if (selectedItems.length < defaultConfig.minSelectedItems) {
      setMessage(`Please choose at least ${defaultConfig.minSelectedItems} kink or fetish.`);
      return;
    }

    const standardItems = selectedItems.filter((item) => catalogSet.has(item));
    const customItems = selectedItems.filter((item) => !catalogSet.has(item));
              ...
              <div>
                {isEntryMode ? (
                  <>
                    ...
                    <p className="mt-2 text-4xl md:text-5xl text-slate-400 font-medium">
                      {appConfig.venueName}
                    </p>
                  </>
                ) : (
                ...
                <div className="mt-4 grid gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setActiveEventDisplayId(e.target.value)}
          ...
          entrySuccess ? (
            ...
            <div className="mx-auto max-w-2xl">
              <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-5 shadow-2xl md:p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  Fill out your name, choose your position, say what you are and who you are seeking, and add your kinks and fetishes.
                </p>
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    placeholder="Type who you are, for example: Male, Female, man, woman, etc"
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 outline-none placeholder:text-slate-500 focus:border-amber-400"
                  />
                </div>

                <div className="mt-4">
                  <label className="mb-2 block text-sm font-semibold">I am seeking</label>
                  <input
                    value={seekingInput}
                    onChange={(e) => setSeekingInput(e.target.value)}
                    placeholder="Type who you are seeking, For example: Male, Female, Daddy, Sir"
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 outline-none placeholder:text-slate-500 focus:border-amber-400"
                  />
                </div>
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        addItemValue(itemInput);
                      }
                    }}
                    placeholder="Type in your kinks and fetishes and press enter"
                    className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 outline-none placeholder:text-slate-500 focus:border-amber-400"
                  />
                </div>

                {itemInput.trim() && itemSuggestions.length > 0 && (
                  <div className="mt-3 max-h-[220px] overflow-auto rounded-2xl border border-slate-800 bg-slate-950/70 pr-1">
                    <div className="divide-y divide-slate-800">
                  ...
                  {selectedItems
                    ...
                    .map((item) => (
                      <button
                        ...
                        className="rounded-full border border-amber-400/40 bg-amber-400/10 px-3 py-1.5 text-sm text-amber-100"
                      >
                        {item} ×
                      </button>
                    ))}
